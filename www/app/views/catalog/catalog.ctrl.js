(function() {
    ...
    angular.module('chasqui').controller('CatalogCtrl', CatalogCtrl);
    
    function CatalogCtrl($stateParams, $scope, $rootScope, $ionicActionSheet, $state, $ionicModal, $ionicSlideBoxDelegate, $sce, $ionicPopup, privateService, publicService, LxNotificationService, destacados, categorias, productores, medallas) {

        $rootScope.idvendedor = $stateParams.idVendedor;
        $scope.idVendedor = $stateParams.idVendedor;
        $rootScope.nombreVendedor = $stateParams.nombreVendedor;
        $scope.nombreVendedor = $stateParams.nombreVendedor;
        $scope.categorias = categorias.data;
        $scope.productores = productores.data;
        $scope.medallas = medallas.data;
        $scope.pss = destacados.data.productos;
        $scope.pagina = 0;
        ...
        $scope.loadMoreData = function() {
            $scope.pagina ++;
            publicService.obtenerProductosSinFiltro($scope.idVendedor, $scope.pagina)
                .success(function(data){
                    ejecutar(data);
                })
                .error(function(data){
                    LxNotificationService.error('Error al obtener productos sin filtro');
                })
        }
